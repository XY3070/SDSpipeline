...
def parse_args():
    ...
    parser.add_argument(
        "--maf-bin-edges",
        type=str,
        default="0.01,0.06,0.10,0.20,0.30,0.40,0.50",
        help="Comma-separated MAF bin edges for normalization (default: 0.01,0.06,0.10,0.20,0.30,0.40,0.50).",
    )
    parser.add_argument(
        "--region-gap",
        type=int,
...
def maf_bin_start(maf: float, bin_edges: list):
    if not math.isfinite(maf) or maf < bin_edges[0] or maf >= bin_edges[-1]:
        return None
    idx = bisect.bisect_right(bin_edges, maf) - 1
    return bin_edges[idx]


def maf_bin_end(bin_start: float, bin_edges: list):
    idx = bin_edges.index(bin_start)
    return bin_edges[idx + 1]


def maf_bin_id(bin_start: float, bin_edges: list):
    be = maf_bin_end(bin_start, bin_edges)
    return f"[{bin_start:.2f},{be:.2f})"


def build_daf_bin_edges(fine_width=0.005, coarse_width=0.01, fine_threshold=0.1):
    edges = []
    v = 0.0
    while v < fine_threshold - 1e-12:
        edges.append(round(v, 6))
        v += fine_width
    while v < (1.0 - fine_threshold) + 1e-12:
        edges.append(round(v, 6))
        v += coarse_width
    while v < 1.0 - 1e-12:
        edges.append(round(v, 6))
...
def first_pass(files, maf_threshold: float, bin_edges: list, exclude_regions: dict, bin_mode: str = "maf"):
    bin_accumulators = {}
    ...
    for _, chrom, path in files:
        ...
        for batch in reader:
            ...
            if bin_mode == "daf":
                binning_af = daf[valid_common][common_mask]
            else:
                binning_af = common_maf

            for rsds_value, af_value in zip(common_rsds, binning_af):
                bin_start = maf_bin_start(float(af_value), bin_edges)
    ...
    for bin_start, (sum_rsds, sumsq_rsds, count) in sorted(bin_accumulators.items()):
        ...
        if not math.isfinite(sd_common) or sd_common <= 0.0:
            raise SystemExit(
                f"Common-variant SDS standard deviation is zero or non-finite in MAF bin {maf_bin_id(bin_start, bin_edges)}"
            )
        bin_stats[bin_start] = BinStats(
            bin_id=maf_bin_id(bin_start, bin_edges),
            af_start=bin_start,
            af_end=maf_bin_end(bin_start, bin_edges),
...
def second_pass(
    ...
    cmplot_tsv = output_prefix.with_name(output_prefix.name + ".cmplot_input.tsv")
    ...
    with normalized_tsv.open("w", newline="") as norm_handle, plot_tsv.open(
        "w", newline=""
    ) as plot_handle, cmplot_tsv.open("w", newline="") as cmplot_handle:
        norm_writer = csv.writer(norm_handle, delimiter="\t")
        plot_writer = csv.writer(plot_handle, delimiter="\t")
        cmplot_writer = csv.writer(cmplot_handle, delimiter="\t")
        ...
        cmplot_writer.writerow(["SNP", "CHR", "BP", "P"])
        ...
        for _, chrom, path in files:
            ...
            for batch in reader:
                ...
                for i in range(rows):
                    bin_start = maf_bin_start(float(binning_af[i]), bin_edges)
                    if bin_start is None:
                        continue
                ...
                for sid, bp, pv in zip(plot_ids, plot_positions, plot_pvals):
                    cmplot_writer.writerow([str(sid), int(chrom) if chrom.isdigit() else chrom, int(bp), f"{max(pv, 1e-300):.15g}"])
    ...
    return normalized_tsv, stats_tsv, plot_tsv, cmplot_tsv, regions_tsv, plot_point_count, plot_max_neg_log10_p
...
def main():
    ...
    if bin_mode == "daf":
        bin_edges = build_daf_bin_edges()
    else:
        bin_edges = sorted(float(x) for x in args.maf_bin_edges.split(","))
        if len(bin_edges) < 2:
            raise SystemExit("--maf-bin-edges must have at least 2 values")
        if bin_edges[0] != args.maf_threshold:
            raise SystemExit(f"First bin edge ({bin_edges[0]}) must equal --maf-threshold ({args.maf_threshold})")
        if bin_edges[-1] > 0.5:
            raise SystemExit("Last bin edge must be <= 0.5")
    ...
    if args.plot_points_tsv:
        ...
        stats = first_pass(files, args.maf_threshold, bin_edges, exclude_regions, bin_mode=bin_mode)
    ...
    with summary_path.open("w", newline="") as handle:
        ...
        writer.writerow(["bin_mode", bin_mode])
        writer.writerow(["bin_edges", ",".join(f"{e:.4g}" for e in bin_edges)])
        ...
        writer.writerow(["cmplot_input_tsv", "" if cmplot_tsv is None else str(cmplot_tsv)])
        writer.writerow(["plot_backend", "CMplot (R)"])
        writer.writerow(
            [
                "summary_warning",
    ...
    print(f"cmplot_input_tsv\t{'' if cmplot_tsv is None else cmplot_tsv}")
