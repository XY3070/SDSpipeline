...
DEFAULT_PROJECT_SINGULARITY = PROJECT_ROOT / "bin" / "singularity"
DEFAULT_PROJECT_SINGULARITY_CONFDIR = PROJECT_ROOT / "third_party" / "singularity_conf"
DEFAULT_PROJECT_SMCPP_ROOTFS = PROJECT_ROOT / "smcpp_latest.sandbox"
DEFAULT_PROJECT_SMCPP_MPLCONFIGDIR = PROJECT_ROOT / "third_party" / "smcpp_matplotlib"
DEFAULT_PROJECT_SMCPP_RUNTIME_DIR = PROJECT_ROOT / ".smcpp_runtime"
DEFAULT_SMC_IMAGE = PROJECT_ROOT / "smcpp_latest.sif"
DEFAULT_MU = 1.25e-8
...
def build_rootfs_smcpp_runner(rootfs: Path, mplconfigdir: Path, rootfs_source: str) -> SmcppRunner:
    ...
    if not mpl_rc.exists():
        raise FileNotFoundError(
            ...
            "Run sds/scripts/prepare_smcpp_runtime.sh first."
        )
    ...
    if rootfs is not None:
        ...
        if mplconfigdir is None:
            raise FileNotFoundError(
                ...
                "Run sds/scripts/prepare_smcpp_runtime.sh first or pass --smcpp-mplconfigdir."
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--subset-samples",
        default=None,
        help="Fixed subset sample file. Defaults to benchmark/demography/<POP>/subset_100.samples.txt.",
    )
    parser.add_argument(
        "--distinguished-pairs",
        default=None,
    ...
    parser.add_argument(
        "--vcf-root",
        default=None,
        help="Directory containing original cohort VCFs. Defaults to sds/data/vcf/<POP>.",
    )
    parser.add_argument(
        "--subset-vcf-dir",
        default=None,
        help="Directory containing or receiving shared subset VCFs. Defaults to benchmark/demography/<POP>/subset_vcf.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Directory for smc++ inputs and outputs. Defaults to benchmark/demography/<POP>/smcpp.",
    )
    parser.add_argument(
        "--smcpp-image",
    ...
    benchmark_root = PROJECT_ROOT / "benchmark" / "demography" / args.pop
