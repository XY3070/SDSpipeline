...
BENCH_ROOT="${BENCH_ROOT:-$PROJECT_ROOT/benchmark/demography}"
SMCPP_IMAGE="${SMCPP_IMAGE:-$PROJECT_ROOT/smcpp_latest.sif}"
SMCPP_CORES="${SMCPP_CORES:-8}"
SMCPP_WINDOW_SIZE="${SMCPP_WINDOW_SIZE:-100}"
...
detect_smcpp_runtime_for_log() {
    ...
    if [[ -x "$PROJECT_ROOT/bin/singularity" ]]; then
        SMCPP_RUNTIME_PATH="$PROJECT_ROOT/bin/singularity"
