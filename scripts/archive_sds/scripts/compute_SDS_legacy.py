#!/usr/bin/env python3
"""
Legacy SDS computation kept for regression checks.
"""
...
class SingletonData:
    ...
    def get_intervals(self, test_position, boundary_upstream, boundary_downstream):
        ...
        for i in range(self.n_individuals):
            ...
            while (
                self.current_indices[i] < n_cols
                and not np.isnan(self.data[i, self.current_indices[i]])
                and self.data[i, self.current_indices[i]] < test_position
            ):
...
class SDSComputer:
    ...
    def compute_neg_log_likelihood(self, params, dat0, dat1, dat2, A1, A2):
        ...
        if n0 > 0:
            ...
            LL += n0 * (
                ...
                + np.mean(self.logsum(np.log(2) + logA1, 0))
            )
        if n2 > 0:
            ...
            LL += n2 * (
                ...
                + np.mean(self.logsum(np.log(2) + logA2, 0))
            )
        if n1 > 0:
    ...
    def process_test_snp(self, snp_id, allele_anc, allele_der, position, genotypes):
        ...
        if (
            np.isnan(upstream).mean() > self.config.skip_boundary_missing_fraction
            or np.isnan(downstream).mean() > self.config.skip_boundary_missing_fraction
        ):
            return None
...
def main():
    ...
    args.add_argument('s_file')
    args.add_argument('t_file')
    args.add_argument('o_file')
    args.add_argument('b_file')
    args.add_argument('g_file')
    args.add_argument('init', type=float)
